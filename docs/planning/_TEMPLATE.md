...
> <https://qiita.com/Y-Y-dev/items/d526fb7cdbe35a3f9384> (2026-08-27 ユーザー指定)。
>
> - **進捗の正本は `docs/task-list.md`**。計画書は個別タスクの詳細 (目的・範囲・条件) を担う。
> - 各フェーズの計画書は本テンプレートの §1〜§8 を必須セクションとし、
>   §9〜§11 (設計詳細・Gotchas・実績) を必要に応じて増減する。
>   (§9〜§11 は旧計画書の良い点 — 詳細仕様・はまりどころ記録・実測値 — を継承する場所)
...
4. タスク ID を含むコミット (例: `feat(P11-A): …`) を作成
5. 証拠中心の完了報告 (結果 / テスト件数 / Git SHA / 残事項)

## 9. サブタスク分割

<!-- 大きなタスクはサブタスクに分割し、task-list.md に行を設ける。
     旧計画書の「サブフェーズ一覧」形式 (テーマ・成果物・依存・想定時間) を踏襲 -->
...
<!-- データ構造・API・コンポーネント設計・判断理由。
     実装後も「なぜそうなったか」の記録として残す (旧計画書の良い点) -->
