...
|---|---|---|---|---|---|
| 2-1 | 重複コンテンツ対策 (モーダル直接ページの noindex) | 🥇 高 | 高 (実害防止) | 極小 | **SEO-2 実施** (`080ede1`) |
| 2-2 | 構造化データ (JSON-LD / schema.org) | 🥇 高 | 高 (最大の伸び代) | 中 | **SEO-1 実施** (`52bf0b9`) |
| 2-3 | 動的 OGP 画像 (`next/og` 1200×630) | 🥇 高 | 高 (CTR) | 中 | **SEO-1 実施** (`52bf0b9`) |
| 2-4 | パンくず表示 + `BreadcrumbList` | 🥈 中 | 中 | 小 | **SEO-1 実施** (`52bf0b9`) |
| 2-5 | sitemap 拡充 (全 4 型 + lastmod) | 🥈 中 | 中 | 小 | **SEO-1 実施** (`52bf0b9`) |
| 2-6〜 | (低優先: 見出し構造・内部リンク強化等) | 🥉 低 | 低 | 小 | **SEO-1 実施** (h1 / カテゴリ `?q=`。facet ではない) |
...
*本レジストリは候補の記録。2-1〜2-6 は SEO-2 / SEO-1 として実施済み（ローカル検証済み）。
本番 HTML 目視はユーザー延期。計画の正本は PHASE13_PLAN.md。*
