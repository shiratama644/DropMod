...
| P12-E2E | Sync の E2E spec (成功 / 失敗 / 復帰) | **CI 実行待ち** (再検証) | 85% | P12-B | CI 上で mock handle 経由の Sync 成功/失敗/復帰が green | run `33245015014` で **DIAG[preview] n=1 / cur=default-pr (1.20.1 のみ)** を特定 → 原因は Sync ではなく **Profile 作成の永続化競合** (500ms debounce 保存がモーダル直後のページ遷移で失われる) / 修正 `ab5fd0b3`+merge `fcdddb12`: 作成時即時永続化 (`dexieSyncProfiles`)・モーダル `await onCreate`・`/tag/game_version` モック・DIAG に alert+pageerror 追加 / 1216 tests green・**CI 再実行はユーザー (workflow_dispatch)** |
